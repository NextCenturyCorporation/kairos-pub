...
import { GetEventTreeRequest, SaveOrUpdateRequest } from "clotho-api";
...
export default class SimpleVis extends Mixins(BaseComponent) {
  ...
  @Prop({ required: false, default: "" }) readonly eventComplexId!: string;
  ...
  @Watch("eventComplex")
  private getEventComplex(): any {
    let request: GetEventTreeRequest = {
      namedGraph: this.namedGraph,
      eventName: this.eventComplexId
    };
    this.$store
      .dispatch("visualization/retrieveEventComplex", request)
      .then(() => {
        this.jsonData = this.$store.getters["visualization/eventComplex"];
        this.getTransformedData();
      })
      .catch(errorStatus => {
        this.handleErrorStatus(errorStatus);
      });
    return this.$store.getters["visualization/eventComplex"];
  }

  private async getTransformedData() {
    var parsedData = await visualizationParser.transformData(this.jsonData);
  ...
  private beforeMount(): void {
    ...
    let gojsKey = process.env.VUE_APP_GOJS_LICENSE_KEY;
  ...
  private getRootNode(): void {
    ...
    if (this.selectedEvent.key === "") {
      this.$alert("", "There is no top level event defined", "warning");
    }
  }
  ...
  updateRelations(e: any, obj: any): void {
    ...
    if (task.relations.length > 0) {
      ...
      var titleSuffix =  relation["http://schema.org/name"] ? (": " + relation["http://schema.org/name"][0]["@value"]) : ""
      var title = "Instantiated" + titleSuffix;
